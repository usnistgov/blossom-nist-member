...
func TestInitBlossom(t *testing.T) {
	...
	chaincodeStub.GetStateReturns([]byte{1, 1, 1}, nil)
	state, err := chaincodeStub.GetState("graph")
	fmt.Println(state)
	state, err = chaincodeStub.GetState("test")
	fmt.Println(state)
}
