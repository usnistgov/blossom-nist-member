...
func TestPolicy(t *testing.T) {
	...
	if _, err := graph.CreateNode("assets_pc", pip.PolicyClass, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := graph.CreateNode("assets", pip.ObjectAttribute, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := graph.CreateNode("reader", pip.UserAttribute, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := graph.CreateNode("writer", pip.UserAttribute, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := graph.CreateNode("creator", pip.UserAttribute, nil); err != nil {
		t.Fatal(err)
	}

	// testUser1
	testUser1 := "eDUwOTo6Q049dGVzdFVzZXIxLE9VPWNsaWVudDo6Q049T3JnMSBDQQ==:Org1MSP"
	if _, err := graph.CreateNode(testUser1, pip.User, nil); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := graph.CreateNode(org1Admin, pip.User, nil); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := graph.CreateNode("o1", pip.Object, nil); err != nil {
		t.Fatal(err)
	}
