...
func TestFilterLicense(t *testing.T) {
	...
	require.NoError(t, err)
	l1, err := graph.CreateNode(licensepap.LicenseObjectAttribute("test-license-1"), pip.ObjectAttribute, nil)
	require.NoError(t, err)
	l2, err := graph.CreateNode(licensepap.LicenseObjectAttribute("test-license-2"), pip.ObjectAttribute, nil)
	require.NoError(t, err)
	...
	err = graph.Associate("ua1", "oa1", pip.ToOps(operations.ViewLicense))
	require.NoError(t, err)
	err = graph.Associate("ua1", "oa2", pip.ToOps(operations.ViewLicense, operations.ViewAllKeys,
		operations.ViewAvailableKeys, operations.ViewCheckedOut))
	require.NoError(t, err)
	...
	licenses, err = NewLicenseDecider().FilterLicenses(transactionContext, licenses)
	require.NoError(t, err)
	require.Equal(t, 2, len(licenses))
	...
	require.Equal(t, []string{}, license.AllKeys)
	require.Equal(t, []string{}, license.AvailableKeys)
	...
	require.Equal(t, 4, license.Available)
	require.Equal(t, float64(99), license.Cost)
	require.Equal(t, []string{"1", "2", "3", "4", "5"}, license.AllKeys)
	require.Equal(t, []string{"2", "3", "4", "5"}, license.AvailableKeys)
